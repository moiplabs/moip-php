...
class MoIP {
	 ...
	 * @access private
	 */
	private $delivery_type_time = array(
		'corridos' => 'Corridos',
		'uteis' => 'Uteis'
	);
	/**
	...
	public function setPagamentoDireto($params) {
		...
		if (
			($params['forma'] == 'debito' or $params['forma'] == 'cartao_credito') and
			(! isset($params['instituicao']) or ! isset($this->institution[$params['instituicao']]))
		) {
			throw new InvalidArgumentException("Você deve especificar uma instituição de pagamento válida quando" . " a forma de forma de pagamento é via débito ou cartao");
		}

		if (
			$params['forma'] == 'cartao_credito' and
		...
		if ($params['forma'] == 'cartao_credito') {
			...
			$portador->addChild('Identidade', $params['cartao']['portador']['identidade_numero'])->addAttribute('tipo', $params['cartao']['portador']['identidade_tipo']);
	...
	public function setCredential($credential) {
		if (! isset($credential['token']) or ! isset($credential['key']) or strlen($credential['token']) != 32 or strlen($credential['key']) != 40)
	...
	public function addDelivery($params) {
		...
		if ($params['tipo'] == 'correios') {
			if ((! isset($params['correios']) or empty($params['correios']))) {
				throw new InvalidArgumentException('É necessário especificar os parâmetros dos correios quando o tipo de frete é Correios');
			}

			if (! isset($params['correios']['peso']) or ! isset($params['correios']['forma_entrega'])) {
				throw new InvalidArgumentException('É necessário passar os parâmetros dos correios quando a forma de envio são os Correios');
			}
		} else {
		...
		$calculo_frete->addChild('Prazo', $params['prazo']['dias'])->addAttribute('Tipo', $this->delivery_type_time[$params['prazo']['tipo']]);
	...
	public function send($client = null) {
		...
		$this->answer = $client->send($this->credential['token'] . ':' . $this->credential['key'], $this->getXML(), $url);
	...
	public function verifyPagamentoDireto($login_moip, $client = null) {
		...
		return (object)array('erro' => $answer->erro, 'id' => (string)$xml->Resposta->ID, 'sucesso' => $xml->Resposta->Status == 'Sucesso', 'carteira_moip' => $xml->Resposta->CarteiraMoIP == 'true', 'cartao_credito' => $xml->Resposta->CartaoCredito == 'true', 'cartao_debito' => $xml->Resposta->CartaoDebito == 'true', 'debito_bancario' => $xml->Resposta->DebitoBancario == 'true', 'financiamento_bancario' => $xml->Resposta->FinanciamentoBancario == 'true', 'boleto_bancario' => $xml->Resposta->BoletoBancario == 'true', 'debito_automatico' => $xml->Resposta->DebitoAutomatico == 'true');
	}
	...
	public function verifyParcelValues($login_moip, $total_parcels, $rate, $simulated_value, $client = null) {
		...
		$return = array('sucesso' => (bool)$xml->Resposta->Status == 'sucesso', 'id' => (string)$xml->Resposta->ID, 'parcelas' => array());
